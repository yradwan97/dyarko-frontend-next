        ...
        <div className="flex w-full h-3/4 flex-col justify-center items-center mt-8 p-6 border border-gray-200 rounded-lg">
            {paymentStatus === 'success' ? (
                ...
                <div className='flex flex-col justify-center items-center'>
                    ...
                    <div className='mt-5 flex flex-col justify-center space-y-1 p-2 items-center border border-main-300 rounded-lg'>
                        <Typography as='h2' className='text-center mb-3' variant='body-md-medium'>Invoice Items</Typography>
                        <Line className="mb-4" />
                        {invoiceDetails.map((item, index) => (
                            <div className='space-x-1 flex flex-row' key={index}>
                                <p className='capitalize'>{item.description}: </p>
                                <p className='text-main-yellow-600 ml-3'> KD {Math.abs(item.amount)}</p>
                            </div>
                        ))}
                        <Line className='mt-2' />
                        <div className='space-x-1 flex flex-row'>
                            <p>Total: </p>
                            <p className='text-main-yellow-600 ml-3'> KD {invoiceDetails.reduce((a, b) => a + Math.abs(b.amount), 0)}</p>
                        </div>
                    </div>
                    <PaymentButton
                        className="mt-4 w-[50%]"
